...
| 2 | loopback UDP round-trip 内核测：`test_net.cpp` 加 `test_udp_loopback`（127.0.0.1 UDP send→单次 poll→listener 收到 payload+端口，确定性躲 SLIRP） | ⏳ | | run-kernel-test-all 两 leg 绿（+1） |
