...
| 批 | 范围 | 状态 | Commit | 测试 |
|----|------|------|--------|------|
| 批1 | **TLS（fs_base）**：`CpuContext` 加 `fs_base`(offset 80, sizeof 80→88)+static_assert；context_switch.S 存/恢复 fs_base(MSR_FS_BASE=0xC0000100) 与 gs_base 同段；`kernel/arch/x86_64/tls.{hpp,cpp}` `cinux::arch::set/get_tls_base`(wrmsr/rdmsr)；fork child ctx 显式 fs_base；单测 round-trip | 🔄 | — | — |
