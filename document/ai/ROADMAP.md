...
| F12 | 开发者生态（**用户决策:砍 Lua/TinyCC 自建,GCC 自举**） | M1 GDB/KALLSYMS✅(KALLSYMS 合 F-INFRA,GDB 待) M2 GCC 自举🔄(批4 启动:glibc 动态工具链;B0 内核 glibc 兼容 auxv+AT_PLATFORM/HWCAP+PT_GNU_STACK+brk 240MB ✅;as+ld→cc1→gcc hello.c 自举) M3 自举闭环⏳(在 CinuxOS 编 CinuxOS) M4 包管理+编辑器⏳ | 现成 GCC/binutils 运行→自举编译→日常可用 |
