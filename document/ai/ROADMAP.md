...
| F12 | 开发者生态（**用户决策:砍 Lua/TinyCC 自建,GCC 自举**） | M1 GDB/KALLSYMS✅(KALLSYMS 合 F-INFRA,GDB 待) M2 GCC 自举🔄(批4-a as+ld 自举 ✅ 2026-07-02:glibc 动态 ELF 跑通,as+ld 编 hello + ./hello 自举两腿 PASS;B0 内核 glibc 兼容 auxv/PT_GNU_STACK/brk + pread64 + mkfs-d 装盘 + gcc 子集;B4-b cc1+headers→B4-c gcc hello.c 完整自举待续) M3 自举闭环⏳(在 CinuxOS 编 CinuxOS) M4 包管理+编辑器⏳ | 现成 GCC/binutils 运行→自举编译→日常可用 |
