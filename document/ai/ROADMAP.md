...
## 当前焦点
**5 弧并发波次 ✅ 全合 main（2026-06-30，PR#51-55）**：F6-M2 ProcFS(#51) / ext2 double-indirect(#52) / F7-M5 TCP(#53) / F5-M4 HPET+RTC(#54) / F8-M1 Pipe增强+M2 FIFO(#55)。post-hoc 审核（5 agent 逐弧读真码对 DIRECTIVES）：**4 弧 clean**（TCP/HPET-RTC/ext2/Pipe 致命 gotcha 全规避），**1 个 HIGH 债 DEBT-022**（ProcFS /proc TOCTOU UAF——DEBT-002 修后 task 真释放，旧「永不释放」契约失效，find_by_pid 锁外解引用 Task*）待修，详见 [debt.md](todo/quality/debt.md) DEBT-022。

**下一步候选**（按依赖就绪度）：
- 🚩 **DEBT-022 ProcFS TOCTOU 修复**（优先，real UAF）：锁内 snapshot Task 字段，对齐 killpg 范式，不动 registry 接口。
- **F7-M6 Socket API**：TCP/UDP 协议层就位，加 sys_socket/bind/connect/recvfrom + production net_init 注册 TCP（M5 留的 production wiring）。
