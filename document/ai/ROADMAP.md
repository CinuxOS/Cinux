...
## 当前焦点
**F2-M4 Page Cache ✅ 完成**（2026-06-17：`PageCache` 256-bucket hash + `get_page` 锁外读/锁内 insert + `handle_pf` 文件感知 demand-read + direct-map 复用，722→730（+8）；Test A 真盘 cache 读 + Test B PF round-trip 端到端验证；详见 `PLAN.md`/`document/notes/2026-06-17-f2-m4-page-cache.md`）。F2 进度 4/7（M1-M4 ✅）。下一焦点待定——F2-M5 Demand Paging 重构（高风险 PF 真 segfault）/ F4 SMP / F5-M2 VirtIO，或 `/milestone` 拆批。
