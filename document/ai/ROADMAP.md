...
| F4 | SMP 多核 | M1 ACPI✅ M2 APIC✅ M3 AP启动✅ M4 多核调度⏳ M5 同步原语⏳ | ACPI+LAPIC+IOAPIC+PIC→APIC切换 ✅；per-CPU 架构(GS/gdt_blocks/swapgs)✅；IPI+trampoline+AP boot ✅(-smp 2 双核 online+idle)；AP 跑任务/runq 并发待 M4 |
...
下个焦点：**F4-M4 多核调度**（per-CPU runq + work stealing + Scheduler::current_ per-CPU 化，让 AP 真跑任务）/ M5 同步原语（原子引用计数 R3 + 锁序图 R6-Part2，F-INFRA 划归）；或暂停 push/PR 验证 F4-M3 Phase 2（trampoline + AP boot，feat/f4-m3-trampoline 3 commit 未 push）。
